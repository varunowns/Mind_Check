...
const activityRank = { none: 0, light: 1, moderate: 2, intense: 3 };
const socialRank = { isolated: 0, normal: 1, social: 2 };
const mealRank = { skipped: 0, mixed: 1, balanced: 2 };
const meetingRank = { "0": 0, "1-2": 1, "3-5": 2, "6+": 3 };
const meetingBuckets = ["0", "1-2", "3-5", "6+"];
const average = (values) => values.length ? Math.round(values.reduce((sum, value) => sum + value, 0) / values.length) : 0;
const preciseAverage = (values) => values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : 0;
...
const addDays = (value, amount) => toDateKey(new Date(parseDateKey(value).getTime() + (amount * DAY)));
const formatRangeLabel = (start, end) => `${start.slice(5)}-${end.slice(5)}`;
const sortDateAsc = (left, right) => left.localeCompare(right);
const sortDateDesc = (left, right) => right.localeCompare(left);
const getStressScore = (checkIn) => "stressScore" in checkIn ? checkIn.stressScore : calculateStressScore(checkIn).score;
const getSubmittedTime = (checkIn) => {
...
export const aggregateDailyCheckIns = (checkIns) => {
...
const getCoveredDates = (checkIns, shieldHistory) => {
    const covered = new Set(checkIns.map((item) => item.date));
    shieldHistory.forEach((item) => covered.add(item.usedOn));
    return covered;
};
export const calculateCurrentStreak = (checkIns, shieldHistory, today = toDateKey()) => {
...
export const calculateSleepDebt = (checkIns, recommendedSleepHours, today = toDateKey()) => {
...
export const calculateBoundaryScore = (checkIns, lateThreshold, today = toDateKey()) => {
...
export const calculateMeetingsVsStress = (checkIns) => {
...
export const getActiveEvent = (events, today = toDateKey()) => {
    return [...events]
        .filter((event) => event.date >= today && event.date <= addDays(today, 7))
        .sort((left, right) => sortDateAsc(left.date, right.date))[0] ?? null;
};
export const calculatePreExamStressPatterns = (checkIns, events, today = toDateKey()) => {
    const daily = aggregateDailyCheckIns(checkIns);
    return [...events]
...
export const calculateBurnoutStatus = (checkIns, burnoutActive = false) => {
    const daily = aggregateDailyCheckIns(checkIns);
    if (!daily.length) {
    ...
    for (let index = daily.length - 1; index >= 0; index -= 1) {
        ...
        if (!isConsecutive && highStreak.length)
            break;
    ...
    for (let index = daily.length - 1; index >= 0; index -= 1) {
        ...
        const previous = daily[index - 1];
        const isConsecutive = !previous || addDays(previous.date, 1) === current.date;
        if (!isConsecutive && lowStreak.length)
            break;
...
const getBadges = (checkIns, streakCount) => {
    const daily = aggregateDailyCheckIns(checkIns);
    const earlyBirdDays = new Set(checkIns
        .filter((item) => {
        const time = getSubmittedTime(item);
        return time.hours < 9;
    })
...
const buildHeatmap = (checkIns, today = toDateKey()) => {
    const daily = aggregateDailyCheckIns(checkIns);
    const dailyMap = new Map(daily.map((item) => [item.date, item]));
...
const buildTrend = (checkIns, pulses, today = toDateKey()) => {
    const daily = aggregateDailyCheckIns(checkIns);
    const dailyMap = new Map(daily.map((item) => [item.date, item]));
...
const buildWeeklyAverages = (checkIns, today = toDateKey()) => {
    const daily = aggregateDailyCheckIns(checkIns);
...
const numericMeeting = (value) => value ?? 0;
const buildInsight = (key, label, messageLabel, values) => {
    const filtered = values.filter((item) => item.metric !== null);
...
export const calculateInsights = (checkIns) => {
    const daily = aggregateDailyCheckIns(checkIns);
    const threshold = 30;
    const comparisons = [
    ...
    ].filter((item) => Boolean(item)).sort((left, right) => right.improvementPercent - left.improvementPercent);
    return {
...
export const summarizePulseDay = (pulses) => {
    return {
